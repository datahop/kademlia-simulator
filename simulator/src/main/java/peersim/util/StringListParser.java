...
/**
 * This utility class can be used to parse range expressions. In particular, it is used by {@link
 * peersim.rangesim.RangeSimulator} to express ranges for configuration properties.
 *
 * <p>The language for range expression is the following:
 *
 * <pre>
 ...
 * </pre>
 *
 * where <tt>value</tt>, <tt>min</tt>, <tt>max</tt> and <tt>step</tt> are numeric atoms that defines
 * ranges.
 *
 * <p>For example, the following range expression:
 *
 * <pre>
 *   5,9:11,13:17|2,32:128*|2
 * </pre>
 *
 * corresponds to 5 (single value), 9-10-11 (range between 9 and 11, default increment 1), 13-15-17
 * (range between 13 and 17, specified step 2, 32-64-128 (range between 32 and 128, multiplicative
 * step 2).
 *
...
public class StringListParser {
   ...
   * @return an array of strings containing all the values defined by the range string
   */
  ...
  private static void parseItem(List<String> list, String item) {
    ...
    if (array.length == 1) {
      ...
      throw new IllegalArgumentException(
          "Element " + item + "should be formatted as <start>:<stop> or <value>");
    }
  }
  ...
  private static void parseRangeItem(List<String> list, String start, String stop) {
    ...
    if (vstart instanceof BigInteger
        && vstart instanceof BigInteger
        && vinc instanceof BigInteger) {
      long vvstart = vstart.longValue();
      long vvstop = vstop.longValue();
      long vvinc = vinc.longValue();
      if (sum) {
        for (long i = vvstart; i <= vvstop; i += vvinc) list.add("" + i);
      } else {
        for (long i = vvstart; i <= vvstop; i *= vvinc) list.add("" + i);
      }
    } else {
      ...
      if (sum) {
        for (double i = vvstart; i <= vvstop; i += vvinc) list.add("" + i);
      } else {
        for (double i = vvstart; i <= vvstop; i *= vvinc) list.add("" + i);
      }
    }
