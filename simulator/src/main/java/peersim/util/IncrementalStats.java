 ...
 * That is, when adding a new data item, it updates the statistics.
 */
public class IncrementalStats {
  ...
  /**
   * Resets the statistics to reflect the zero elements set. Min and max are set to positive and
  ...
  /**
   * Updates the statistics according to this element. It calls <code>add(item,1)</code>.
   *
   * @see #add(double,int)
   */
  ...
  /** Updates the statistics assuming element <code>item</code> is added <code>k</code> times. */
  ...
  /**
   * The empirical variance of the data items. Guaranteed to be larger or equal to 0.0. If due to
   * rounding errors the value becomes negative, it returns 0.0.
   */
  ...
  /**
   * Prints the following quantities separated by spaces in a single line in this order. Minimum,
   * maximum, number of items, average, variance, number of minimal items, number of maximal items.
   */
  public String toString() {

    return min + " " + max + " " + n + " " + sum / n + " " + getVar() + " " + countmin + " "
        + countmax;
  }
}
