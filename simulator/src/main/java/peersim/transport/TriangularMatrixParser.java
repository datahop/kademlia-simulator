...
/**
 * Initializes static singleton {@link E2ENetwork} by reading a trace file containing the latency
 * distance measured between a set of "virtual" routers. Latency is assumed to be symmetric, so the
 * latency between x and y is equal to the latency to y and x.
 *
 * <p>The format of the file is as follows: all values are stored as integers. The first value is
 * the number of nodes considered. The rest of the values correspond to a "strictly upper triangular
 * matrix" (see this <a
 * href="http://en.wikipedia.org/w/index.php?title=Triangular_matrix&oldid=82411128">link</a>),
 * ordered first by row than by column.
 *
...
public class TriangularMatrixParser implements Control {
   ...
   * First, the file is used as a pathname in the local file system. If no file can be identified in
   * this way, the file is searched in the local classpath. If the file cannot be identified again,
  ...
  public boolean execute() {
    try {
      ...
      try {
        ...
        System.err.println(
            "TriangularMatrixParser: Reading " + filename + " from local file system");
      } catch (FileNotFoundException e) {
        in =
            new ObjectInputStream(
                new BufferedInputStream(ClassLoader.getSystemResourceAsStream(filename)));
        System.err.println(
            "TriangularMatrixParser: Reading " + filename + " through the class loader");
      }
