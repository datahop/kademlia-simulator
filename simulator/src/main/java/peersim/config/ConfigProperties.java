...
public class ConfigProperties extends Properties {
   ...
   * <code>resource</code> is used to attempt loading default values from the given system resource.
   * Then all Strings in <code>pars</code> are processed in the order they appear in the array. For
   * <code>pars[i]</code>, first a property file with the name <code>pars[i]</code> is attempted to
   * be loaded. If the file does not exist or loading produces any other IOException, <code>pars[i]
   * </code> is interpreted as a property definition, and it is set.
   *
   * <p>A little inconvenience is that if <code>pars[i]</code> is supposed to be a command line
   * argument, but it is a valid filename at the same time by accident, the algorithm will process
   * it as a file instead of a command line argument. The caller must take care of that.
   *
   * <p>No exceptions are thrown, instead error messages are written to the standard error. Users
   * who want a finer control should use the public methods of this class.
   *
   * @param pars The (probably command line) parameter list.
   * @param resource The name of the system resource that contains the defaults. null if there isn't
   *     any.
   */
  ...
  /**
   * Loads given file. Calls <code>Properties.load</code> with a file input stream to the given
   * file.
   */
  ...
  /**
   * Appends a property defined in the given string. The string is considered as a property file
   * line. It is converted to a byte array according to the default character encoding and then
   * loaded by the <code>Properties.load</code> method. This means that the ISO-8859-1 (or
   * compatible) encoding is assumed.
   */
