...
public class ParsedProperties extends ConfigProperties {
   ...
   * property names using <code>{</code> and <code>}</code>.
   *
   * <p>When a bracket is present, it must be the only non-space element of a line. The last
   * property defined before the opening bracket define the prefix that is added to all the
   * properties defined included between brackets. In other words, a construct like this:
   *
   * <pre>
   ...
   * </pre>
   *
   * <p>Know limitations: The parsing mechanism is very simple; no complex error messages are
   * provided. In case of missing closing brackets, the method will stop reporting the number of
   * missing brackets. Additional closing brackets (i.e., missing opening brackets) produce an error
   * messages reporting the line where the closing bracket is present. Misplaced brackets (included
   * in lines that contains other characters) are ignored, thus may indirectly produce the previous
   * error messages.
   */
  ...
  private int parseStream(BufferedReader f, String prefix, int pars, int lines, Set prefixes)
    ...
    if (prefixes.contains(prefix)) {
      System.err.println(
          "Error at line "
              + lines
              + ": multiple bracket "
              + "blocks referring to the same configuration entry "
              + prefix);
      System.exit(1);
    } else {
      prefixes.add(prefix);
    ...
    while ((part = f.readLine()) != null) {
      ...
      if (line.equals("{")) {
        ...
        if (pars == 0) {
          System.err.println(
              "Error: Additional } at line " + lines + " when parsing the configuration file");
          System.exit(1);
        }
        return lines;
      } else {
