...
public class ConfigContainer {
  ...
  /**
   * Map associating string protocol names to the numeric protocol identifiers. The protocol names
   * are understood without prefix.
   */
  ...
  /**
   * If true, no exception is thrown. Instead, an error is printed and the Configuration tries to
   * return a reasonable return value
   */
  ...
  public ConfigContainer(Properties config, boolean check) {
    ...
    if (Configuration.DEBUG_EXTENDED.equals(debug)) debugLevel = DEBUG_CONTEXT;
      ...
      while (i.hasNext()) {
        ...
        System.err.println(
            "DEBUG " + name + ("".equals(map.get(name)) ? "" : " = " + map.get(name)));
      }
    } else if (debug != null) {
  ...
  private boolean getBool(String name) {
    ...
    if (config.getProperty(name).matches("\\p{Blank}*")) {
      throw new MissingParameterException(
          name, "Blank value is not accepted when parsing Boolean.");
    }
  ...
  private Number getVal(String initial, String property, int depth) {
    if (depth > maxdepth) {
      throw new IllegalParameterException(
          initial, "Probable recursive definition - exceeded maximum depth " + maxdepth);
    }
  ...
  public String lookupPid(int pid) {
    ...
    for (Map.Entry<String, Integer> i : protocols.entrySet()) {
      if (i.getValue().intValue() == pid) return i.getKey();
    }
  ...
  private Class getClazz(String name) {
    ...
    if (c == null) {
      ...
      if (fullname != null) {
        throw new IllegalParameterException(
            name, "Class " + classname + " does not exist. Possible candidate(s): " + fullname);
      }
    }
    if (c == null) {
  ...
  private Object getInst(String name) {
    ...
    try {
      ...
      throw new IllegalParameterException(
          name, "Class " + classname + " has no " + classname + "(String) constructor");
   ...
   * there is no config entry <code>{@value peersim.config.Configuration#PAR_INCLUDE}+"."+name
   * </code> or <code>{@value peersim.config.Configuration#PAR_ORDER}+"."+name</code> then the order
   * is alphabetical. Otherwise this entry defines the order. For more information see {@link
   * Configuration}.
   *
  ...
  public String[] getNames(String name) {
    ...
    while (e.hasMoreElements()) {
      ...
      if (key.startsWith(pref) && key.indexOf(".", pref.length()) < 0) ll.add(key);
    }
  ...
  /**
   * The input of this method is a set of property <code>names</code> (e.g. initializers, controls
   * and protocols) and a string specifying the type (prefix) of these. The output is in <code>names
   * </code>, which will contain a permutation of the original array. Parameter
   * PAR_INCLUDE+"."+type, or if not present, PAR_ORDER+"."+type is read from the configuration. If
   * none of them are defined then the order is identical to that of <code>names</code>. Otherwise
   * the configuration entry must contain entries from <code>names</code>. It is assumed that the
   * entries in <code>names</code> contain only word characters (alphanumeric and underscore '_'.
   * The order configuration entry thus contains a list of entries from <code>names</code> separated
   * by any non-word characters.
   *
   * <p>It is not required that all entries are listed. If PAR_INCLUDE is used, then only those
   * entries are returned that are listed. If PAR_ORDER is used, then all names are returned, but
   * the array will start with those that are listed. The rest of the names follow in alphabetical
   * order.
   *
   * @param names the set of property names to be searched
   * @param type the string identifying the particular set of properties to be inspected
   */
  private String[] order(String[] names, String type) {
    ...
    if (!include) order = getString(Configuration.PAR_ORDER + "." + type, null);
    ...
    if (order != null && !order.equals("")) {
      ...
      for (; i < sret.length; i++) {
        int j = i;
        for (; j < names.length; ++j) if (names[j].equals(type + "." + sret[i])) break;
        if (j == names.length) {
          throw new IllegalParameterException(
              (include ? Configuration.PAR_INCLUDE : Configuration.PAR_ORDER) + "." + type,
  ...
  /**
   * Print debug information for configuration. The amount of information depends on the debug level
   * DEBUG. 0 = nothing 1 = just the config name 2 = config name plus method calling
   *
   * @param name
   */
  private void debug(String name, String result) {
    if (debugLevel == DEBUG_NO) return;
  ...
  /** Among the defined properties, returns the one more similar to String property */
