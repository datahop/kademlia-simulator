...
The Kademlia Simulator revolves around the KademliaProtocol class, which is the core implementation of the Kademlia protocol and spans approximately 700 lines of code. Among its various features, the class incorporates a routing table data structure that monitors nodes within the network. It also comprises functions to include new nodes, locate nodes in the network, and manage events. The protocol defines different message types, such as Find, Put, Get, and Store, that nodes leverage to execute different operations, such as finding the nearest nodes to a specific key, saving data within the network, and responding to messages.

For instance, to handle send and receive messages/events, the class provides the **`sendMessage`** method and the **`handleResponse`** methods. The former sends messages between nodes in the network, while the latter handles incoming events, such as responses to messages, put requests, get requests, and so on. Additionally, the KademliaProtocol class includes several helper classes, such as **`FindOperation`**, which handles find node operations, and Timeout (currently not in use??), which manages timeout events.
