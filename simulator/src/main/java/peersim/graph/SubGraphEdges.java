...
/**
 * This class is an adaptor for representing subgraphs of any graph. The subgraph is defined the
 * following way. The subgraph always contains all the nodes of the original underlying graph.
 * However, it is possible to remove edges by flagging nodes as removed, in which case the edges
 * that have at least one end on those nodes are removed. If the underlying graph changes after
 * initialization, this class follows the change.
 */
public class SubGraphEdges implements Graph {
  ...
  /**
   * If both i and j are within the node set of the subgraph and the original graph has an (i,j)
   * edge, returns that edge.
   */
