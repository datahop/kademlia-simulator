 ...
 * The graph to be made undirected is passed to the constructor. Only the reference is stored.
 * However, at construction time the incoming edges are stored for each node, so if the graph passed
 * to the constructor changes over time then methods {@link #getNeighbours(int)} and {@link
 * #degree(int)} become inconsistent (but only those). The upside of this inconvenience is that
 * {@link #getNeighbours} will have constant time complexity.
 *
 * @see UndirectedGraph
 */
public class ConstUndirGraph implements Graph {
  ...
  /**
   * Initialization based on given graph. Stores the graph and if necessary (if the graph is
   * directed) searches for the incoming edges and stores them too. The given graph is stored by
   * reference (not cloned) so it should not be modified while this object is in use.
   */
  ...
  /**
   * Uses sets as collection so does not support multiple edges now, even if the underlying directed
   * graph does.
   */
  ...
  /**
   * If there is an (i,j) edge, returns that, otherwise if there is a (j,i) edge, returns that,
