...
public class GraphIO {
  ...
  /**
   * Prints graph in edge list format. Each line contains exactly two node IDs separated by
   * whitespace.
   */
  ...
  /**
   * Prints graph in neighbor list format. Each line starts with the id of a node followed by the
   * ids of its neighbors separated by space.
   */
  ...
  public static void writeDOT(Graph g, PrintStream out) {
    ...
    for (int i = 0; i < g.size(); ++i) {
      ...
      while (it.hasNext()) {
        final int j = it.next();
        if (g.directed()) out.println(i + " -> " + j + ";");
        else if (i <= j) out.println(i + " -- " + j + ";");
      }
    }
  ...
  public static void writeGML(Graph g, PrintStream out) {
    ...
    for (int i = 0; i < g.size(); ++i) out.println("node [ id " + i + " ]");
  ...
  /**
   * Saves the given graph to the given stream to be read by NETMETER. It should be ok also for
   * Pajek.
   */
  public static void writeNetmeter(Graph g, PrintStream out) {

    out.println("*Vertices " + g.size());
    for (int i = 0; i < g.size(); ++i) out.println((i + 1) + " \"" + (i + 1) + "\"");

    out.println("*Arcs");
    for (int i = 0; i < g.size(); ++i) {
      Iterator it = g.getNeighbours(i).iterator();
      while (it.hasNext()) {
        out.println((i + 1) + " " + (((Integer) it.next()).intValue() + 1) + " 1");
      }
    }
    out.println("*Edges");
  }
  ...
  /**
   * Saves the given graph to the given stream in Chaco format. We need to output the number of
   * edges so they have to be counted first which might not be very efficient. Note that this format
   * is designed for undirected graphs only.
   */
  public static void writeChaco(Graph g, PrintStream out) {

    if (g.directed()) System.err.println("warning: you're saving a directed graph in Chaco format");
  ...
  /**
   * Read a graph in newscast graph format. The format depends on mode, the parameter. The file
   * begins with the three byte latin 1 coded "NCG" string followed by the int MODE which is the
   * given parameter. The formats are the following as a function of mode:
   *
   * <ul>
   *   <li>1: Begins with cacheSize in binary format (int), followed by the numberOfNodes (int), and
   *       then a continuous series of exactly numberOfNodes records, where a record describes a
   *       node's neighbours and their timestamps. A record is a series of exactly cacheSize
   *       (int,long) pairs where the int is the node id, and the long is the timestamp. Node id-s
   *       start from 1. Node id 0 means no node and used if the parent node has less that cacheSize
   *       nodes.
   * </ul>
   *
   * @param file Filename to read
   * @param direction If 0, the original directionality is preserved, if 1, than each edge is
   *     reversed, if 2 then directionality is dropped and the returned graph will be undirected.
   */
