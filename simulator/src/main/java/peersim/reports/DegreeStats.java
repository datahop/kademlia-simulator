...
public class DegreeStats extends GraphObserver {
  ...
  /**
   * If defined, then the given number of nodes will be traced. That is, it is guaranteed that in
   * each call the same nodes will be picked in the same order. If a node being traced fails, its
   * degree will be considered 0. Not defined by default.
   *
   * @config
   */
  ...
  /**
   * Selects a method to use when printing results. Three methods are known: "stats" will use {@link
   * IncrementalStats#toString}. "freq" will use {@link IncrementalFreq#print}. "list" will print
   * the degrees of the sample nodes one by one in one line, separated by spaces. Default is
   * "stats".
   *
   * @config
   */
  ...
  /**
   * Selects the types of links to print information about. Three methods are known: "live": links
   * pointing to live nodes, "dead": links pointing to nodes that are unavailable and "all": both
   * dead and live links summed. "all" and "dead" require parameter {@value
   * peersim.reports.GraphObserver#PAR_UNDIR} to be unset (graph must be directed). Default is
   * "live".
   *
   * @config
   */
  ...
  private int nextNodeId() {
    if (trace) {
      if (traced == null) {
        ...
        traced = new Node[nn];
        for (int j = 0; j < nn; ++j) traced[j] = Network.get(j);
      }
  ...
  /**
   * Prints statistics about node degree. The format of the output is specified by {@value
   * #PAR_METHOD}. See also the rest of the configuration parameters.
   *
   * @return always false
   */
