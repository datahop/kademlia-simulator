...
public class GraphPrinter extends GraphObserver {
  ...
  /**
   * This is the prefix of the filename where the graph is saved. The extension is ".graph" and
   * after the prefix the basename contains a numeric index that is incremented at each saving
   * point. If not given, the graph is dumped on the standard output.
   *
   * @config
   */
  ...
  /**
   * The name for the format of the output. Defaults to "neighborlist", which is a plain dump of
   * neighbors. The class {@link peersim.dynamics.WireFromFile} can read this format. Other
   * supported formats are "chaco" to be used with Yehuda Koren's Embedder, "netmeter" to be used
   * with Sergi Valverde's netmeter and also with pajek, "edgelist" that dumps one (directed) node
   * pair in each line for each edge, "gml" that is a generic format of many graph tools, and "dot"
   * that can be used with the graphviz package.
   *
