 ...
 * It implements {@link Control} but it will most often be invoked only once for each protocol as an
 * initializer, since the scheduled events schedule themselves for the consecutive executions (see
 * {@link NextCycleEvent}).
 *
 * <p>All {@link CDProtocol} specifications in the configuration need to contain a {@link Scheduler}
 * specification at least for the step size (see config parameter {@value
 * peersim.core.Scheduler#PAR_STEP} of {@link Scheduler}). This value is used as the cycle length
...
public class CDScheduler implements Control, NodeInitializer {
  ...
  /**
   * Parameter that is used to define the class that is used to schedule the next cycle. Its type is
   * (or extends) {@link NextCycleEvent}. Defaults to {@link NextCycleEvent}.
   *
   * @config
   */
  ...
  /**
   * The protocols that this scheduler schedules for the first execution. It might contain several
   * protocol names, separated by whitespace. All protocols will be scheduled based on the common
   * parameter set for this scheduler and the parameters of the protocol (cycle length). Protocols
  ...
  /**
   * If set, it means that the initial execution of the given protocol is scheduled for a different
   * random time for all nodes. The random time is a sample between the current time (inclusive) and
   * the cycle length (exclusive), the latter being specified by the step parameter (see {@link
   * Scheduler}) of the assigned protocol.
   *
   * @see #execute
   * @config
   */
  ...
  /**
   * Contains the scheduler objects for all {@link CDProtocol}s defined in the configuration. The
   * length of the array is the number of protocols defined, but those entries that belong to
   * protocols that are not {@link CDProtocol}s are null.
   */
  ...
  /**
   * Schedules the protocol at all nodes for the first execution adding it to the priority queue of
   * the event driven simulation. The time of the first execution is determined by {@link
   * #firstDelay}. The implementation calls {@link #initialize} for all nodes.
   *
   * @see #initialize
   */
  ...
  /**
   * Returns the time (through giving the delay from the current time) when this even is first
   * executed. If {@value #PAR_RNDSTART} is not set, it returns zero, otherwise a random value
   * between 0, inclusive, and cyclelength, exclusive.
   *
   * @param cyclelength The cycle length of the cycle based protocol for which this method is called
   */
  protected long firstDelay(long cyclelength) {

    if (randstart) return CommonState.r.nextLong(cyclelength);
    else return 0;
  }
}
