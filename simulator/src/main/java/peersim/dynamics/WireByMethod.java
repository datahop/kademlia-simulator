...
/**
 * Takes a {@link Linkable} protocol and adds connections using an arbitrary method. No connections
 * are removed. The connections are added by an arbitrary method that can be specified in the
 * configuration. The properties the method has to fulfill are the following
 *
 * <ul>
 *   <li>It MUST be static
 *   <li>It MUST have a first argument that can be assigned from a class that implements {@link
 *       Graph}.
 *   <li>It MAY contain zero or more arguments following the first one.
 *   <li>All the arguments after the first one MUST be of primitive type int, long or double, except
 *       the last one, which MAY be of type that can be assigned from <code>java.util.Random</code>
 * </ul>
 *
 * The arguments are initialized using the configuration as follows.
 *
 * <ul>
 *   <li>The first argument is the {@link Graph} that is passed to {@link #wire}.
 *   <li>The arguments after the first one (indexed as 1,2,etc) are initialized from configuration
 *       parameters of the form {@value #PAR_ARG}i, where i is the index.
 *   <li>If the last argument can be assigned from <code>java.util.Random</code> then it is
 *       initialized with {@link CommonState#r}, the central source of randomness for the simulator.
 * </ul>
 *
 ...
 * </pre>
 *
 * Note that the {@value #PAR_CLASS} parameter defaults to {@link GraphFactory}, and {@value
 * #PAR_METHOD} defaults to "wire".
 */
public class WireByMethod extends WireGraph {
  ...
  /**
   * The class that has the method we want to use. Defaults to {@link GraphFactory}.
   *
   * @config
   */
