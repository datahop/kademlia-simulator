...
/**
 * This {@link Control} invokes a specified method on a protocol. The method is defined by config
 * parameter {@value #PAR_METHOD} and the protocol is by {@value #PAR_PROT}. The method must not
 * have any parameters and must return void. If no protocol is specified, then the method will be
 * invoked on all protocol in the protocol stack that define it.
 *
 * <p>Although the method cannot have any parameters, it can of course read {@link CommonState}. It
 * is guaranteed that the state is up-to-date, inlcuding eg method {@link CommonState#getNode}.
 */
public class MethodInvoker implements Control, NodeInitializer {
  ...
  /**
   * The protocol to operate on. If not defined, the given method will be invoked on all protocols
   * that define it. In all cases, at least one protocol has to define it.
   *
   * @config
   */
  ...
  public MethodInvoker(String prefix) {
    ...
    if (!Configuration.contains(prefix + "." + PAR_PROT)) {
      ...
      for (int i = 0; i < Network.prototype.protocolSize(); ++i) {
        Method m = null;
        try {
          m = MethodInvoker.getMethod(Network.prototype.getProtocol(i).getClass(), methodName);
      ...
      try {
        method = new Method[1];
        method[0] =
            MethodInvoker.getMethod(Network.prototype.getProtocol(pid[0]).getClass(), methodName);
      } catch (NoSuchMethodException e) {
  ...
  private static Method getMethod(Class cl, String methodName) throws NoSuchMethodException {
    ...
    for (Method m : methods) {
      if (m.getName().equals(methodName)) {
        ...
        if (pars.length == 0 && ret == void.class) list.add(m);
      }
    }

    if (list.size() == 0) {
      throw new NoSuchMethodException("Method " + methodName + " in class " + cl.getName());
    }
