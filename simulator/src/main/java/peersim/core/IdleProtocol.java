...
/**
 * A protocol that stores links. It does nothing apart from that. It is useful to model a static
 * link-structure (topology). The only function of this protocol is to serve as a source of
...
public class IdleProtocol implements Protocol, Linkable {
  ...
  /**
   * Adds given node if it is not already in the network. There is no limit to the number of nodes
   * that can be added.
   */
  public boolean addNeighbor(Node n) {
    for (int i = 0; i < len; i++) {
      if (neighbors[i] == n) return false;
    }
