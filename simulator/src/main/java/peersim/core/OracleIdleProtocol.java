...
/**
 * A protocol that does nothing but knows everything. It provides an interface which models a
 * protocol that knows all nodes in the network, i.e. the neighborhood set of this protocol is
 * always the whole node set. this protocol is also extremely cheap, in fact it has no data fields.
 */
public final class OracleIdleProtocol implements Protocol, Linkable {
  ...
  /**
   * This is an expensive operation, should not be used at all. It returns false only if the given
