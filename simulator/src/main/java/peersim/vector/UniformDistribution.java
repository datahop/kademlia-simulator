...
/**
 * Initializes the values drawing uniform random samples from the range [{@value #PAR_MIN}, {@value
 * #PAR_MAX}[.
 *
 * @see VectControl
 * @see peersim.vector
...
public class UniformDistribution extends VectControl implements NodeInitializer {
  ...
  /**
   * The lower bound of the uniform random variable, inclusive. Defaults to -{@value #PAR_MAX}.
   *
   * @config
   */
  ...
  /**
   * Initializes the values drawing uniform random samples from the range [{@value #PAR_MIN},
   * {@value #PAR_MAX}[.
   *
   * @return always false
   */
  public boolean execute() {
    ...
    if (setter.isInteger()) {
      ...
      for (int i = 0; i < Network.size(); ++i) {
        setter.set(i, CommonState.r.nextDouble() * d + min.doubleValue());
      }
    }
  ...
  /**
   * Initializes the value drawing a uniform random sample from the range [{@value #PAR_MIN},
   * {@value #PAR_MAX}[.
   *
