...
/**
 * Initializes the values so that {@value #PAR_PEAKS} nodes have value {@value #PAR_VALUE}/{@value
 * #PAR_PEAKS}, the rest {@value #PAR_LVALUE} (zero by default).
 *
 * @see VectControl
 * @see peersim.vector
 */
public class PeakDistribution extends VectControl {
  ...
  /**
   * The value for the nodes that are not peaks. This parameter is optional, by default, the nodes
   * that are not peaks are set to zero. This value overrides that behavior.
   *
   * @config
   */
  ...
  /**
   * The number of peaks in the system. If this value is greater than or equal to 1, it is
   * interpreted as the actual number of peaks. If it is included in the range [0, 1] it is
   * interpreted as a percentage with respect to the current network size. Defaults to 1.
   *
   * @config
   */
  ...
  public PeakDistribution(String prefix) {
    ...
    if (setter.isInteger()) {
      ...
      lvalue = new Double(Configuration.getDouble(prefix + "." + PAR_LVALUE, 0));
    }
  }
