...
/**
 * A protocol that is not really a protocol, but a trick to carry out all kinds of tasks during the
 * simulation. Many users will probably not need it, but it is a nice way to e.g. run controls at
 * any time, not only between cycles.
 */
public class DaemonProtocol implements CDProtocol {
  ...
  /**
   * The controls will be run according to this frequency. It is interpreted within a cycle, in
   * terms of cycle time ({@link CDState#getCycleT}). The first cycletime is 0. Defaults to 1.
   *
   * @config
   */
  ...
  /** Runs the configured controls if {@link CDState#getCycleT} % {@value #PAR_STEP}=0. */
