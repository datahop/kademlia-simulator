...
/**
 * It generalizes its superclass so that the list of protocols to run can be specified. The
 * superclass ({@link FullNextCycle}) always runs all the {@link CDProtocol}s.
 */
public class NextCycle extends FullNextCycle {
  ...
  /**
   * Execute the configured protocols on all nodes. It works exactly as {@link
   * FullNextCycle#execute}, only just the configured protocols are iterated over.
   */
  public boolean execute() {
    ...
    for (int j = 0; j < Network.size(); ++j) {
      ...
      for (int pid : pids) {
        ...
        if (!protSchedules[pid].active(cycle)) continue;
