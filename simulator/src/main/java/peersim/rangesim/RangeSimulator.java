...
/**
 * This class is the main class for the Range Simulator. A range is a collection of values
 * <em>S</em> to be assigned to a variable <em>v</em>. The Range Simulator invokes the standard
 * Peersim simulator once for each distinct value. If multiple ranges <em>S1, S2, ..., Sn</em> are
 * specified, the standard Peersim simulator is invoked for each element in <em>S1 * S2 * ... *
 * Sn</em>.
 *
 * <p>Ranges are specified with the following syntax:
 *
 * <pre>
 * range.[id] [var];[range]
 * </pre>
 ...
 *
 * <UL>
 *   <LI>{@value #PAR_RANGE} is the prefix for all range specifications;
 *   <LI><code>id</code> is an identifier; since they are not referred anywhere else, consecutive
 *       numbers are a good choice for range identifiers;
 *   <LI><code>var</code> is a variable parameter
 *   <LI><code>range</code> describes the collection of values to be associated to <code>var</code>,
 *       whose syntax and semantics is defined in {@link peersim.util.StringListParser}.
 * </UL>
 *
 ...
 * </pre>
 *
 * With this specification, the collection of values associated to <code>SIZE</code> is
 * {2^10,2^11,...,2^18}; <code>K</code> contains {20,21,22,...,30}, while <code>CHURN</code>
 * contains just the specified values.
 *
 * <p>Values can be specified as constant expressions (like 2^10, (5+10), etc.) but variables cannot
 * be used.
 *
 * <p>A separate Java virtual machine is invoked to run each of the experiments. An attempt is done
 * to run the same JVM version as the one running the Range Simulator; if this is not possible (for
 * example due to path problems), the command shell mechanism is used to run the first JVM version
 * found in the path. It is possible to specify options for the forked JVM using the {@value
 * #PAR_JVM} parameter on the command line. For example, a command line like this:
 *
 * <pre>
 ...
 *
 * <p>The new JVM inherits the same classpath as the JVM running the RangeSimulator. The {@value
 * #PAR_JVM} parameter can be used to specify additional classpath specification.
 *
...
public class RangeSimulator implements ProcessHandler {
  ...
  /**
   * This config property can be used to set options in the JVMs that are forked to execute
   * experiments with different configuration parameters.
   *
   * @config
   */
  ...
  public RangeSimulator(String[] args) {
    ...
    if (opt == null) jvmoptions = new String[0];
    else jvmoptions = opt.split(" ");
  ...
  /**
   * Parses a collection of range specifications and returns the set of parameter that will change
   * during the simulation and the values that will be used for those parameters.
   */
  private void parseRanges() {
    ...
    if (Configuration.contains(Simulator.PAR_EXPS) || ranges.length == 0) {
      ...
      pars[0] = "EXP";
      values[0] = StringListParser.parseList("1:" + Configuration.getInt(Simulator.PAR_EXPS, 1));
      start = 1;
    } else {
    ...
    for (int i = start; i < pars.length; i++) {
      ...
      if (array.length != 2) {
        throw new IllegalParameterException(
            ranges[i], " should be formatted as <parameter>;<value list>");
      }
      pars[i] = array[0];
  ...
  /**
   * Selects the next set of values by incrementing the specified index array. The index array is
   * treated as a vector of digits; the first is managed managed as a vector of digits.
   */
  ...
  private void doExperiments(String[] args) {
    ...
    String javapath = System.getProperty("java.home") + filesep + "bin" + filesep + "java";
    ...
    // Add the jvm options
    for (int i = 0; i < jvmoptions.length; i++) list.add(jvmoptions[i]);
    ...
    for (int i = 0; i < values.length; i++) list.add("");
    ...
    while (idx[0] < values[0].length) {
      ...
      list.set(startlog, Simulator.PAR_REDIRECT + "." + TaggedOutputStream.PAR_RANGES + "=" + log);
  ...
  /**
   * Execute the "command line" represented by this String list. The first argument is the process
   * to be executed. We try to run the same JVM as the current one. If not possible, we use the
   * first java command found in the path.
   */
  private void executeProcess(List<String> list) {
    ...
    // and System.err.
    BufferedReader toprint = new BufferedReader(new InputStreamReader(p.getInputStream()));
    String line;
  ...
  /**
   * Wait until the java virtual machine has terminated; it won't be used in this class, but you
   * never know.
   */
