 ...
 * augmented with a tag character. This is used to discriminate among standard error and standard
 * output. This feature is needed for launching new JVMs; it should not be used for other purposes.
 *
...
public class TaggedOutputStream extends PrintStream {
  ...
  /**
   * This parameter contains the string that should be printed on each line, containing the values
   * of the range parameters for the experiment which is being run. The full name of this
   * configuration string is prefixed by {@value peersim.Simulator#PAR_REDIRECT}.
   *
   * @config
   */
  ...
  /**
   * This parameter contains the list of observers for which the string contained in parameter
   * {@value #PAR_RANGES} should be augmented with a "TIME &lt;t&gt;" specification regarding
   * current time. Observers are separated by one of this characters: ' ' - ',' - ';'.
   *
   * @config
   */
  ...
  private void printLine() {
    ...
    if (parts.length == 2) {
      ...
      stdout.print(": ");
      stdout.print(ranges);
      if (obstime.contains(parts[0])) stdout.print(" TIME " + CommonState.getTime() + " ");
